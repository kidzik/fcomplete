...
In this section, we present an extension of our univariate framework to multivariate measurements sparse in time. %One major advantage of the matrix-factorization approach is its simple algebraic formulation and solution which makes it easily adaptable to more complex situations.
...
\end{align}
where $X_i$ are some $N \times T$ matrices corresponding to the processes measured on a grid of $T$ points, $B$ is a basis evaluated on the same grid and orthogonalized (a $T \times K$ matrix), and $Z$ is a $N \times pK$ matrix.
...
Let $ \bB = I_p \otimes B $ be the Kronecker product of $p \times p$ identity and $B$, i.e. a $pT \times pK$ matrix with $B$ stacked $p$ times on the diagonal, and let $\bX = (X_{1}: X_{2}:...: X_{p})$. Note that $\bB$ is orthogonal and therefore results developped in Section \ref{ss:reduced-rank} apply here. In particular, singular value thresholding solves
...
% Let $W$ be a latent representation of $X_1, X_2, ..., X_p$, derived using one of the methods above.
Let $U$ be an unknown $N \times d_1$ matrix, $W$ be a $N \times d_2$ orthogonal matrix derived from $(X_1,...,X_p)$, where $d_1, d_2$ are the numbers of latent components. We are searching for $U$, a $d_1 \times K$ matrix $A_1$ and a $d_2 \times K$ matrix $A_2$ solving
\begin{align}\label{eq:pcr}
\minimize_{A,Z} \|P_\Omega(Y - ZB' - WAB')\|_F^2 + \lambda\|Z\|_*,
\end{align}
where $P_\Omega$ is a projection on a set of observed coefficients.

For a known $A_2$, we can find the $UA_1B'$ by applying methodology developped in \label{ss:reduced-rank} to the partially observed matrix $(Y - WA_2B')$. For a given $UA_1B'$, the problem \eqref{eq:pcr} is just a regression of $Y - UA_1B'$ on $W$. Based on this observation we propose an iterative algorithm. We refer to Algorithm \ref{alg:sparse-regression} for the full description of the procedure.
...
\end{flushleft}
\begin{enumerate}
\item For sparsely observed $\bX = (X_1,X_2,...,X_p)$ find a latent scores $W$ (Algorithm \ref{alg:soft-impute})
%\item Define $W = W_1:W_2:...:W_p$
%\item Orthogonalize $W$
\end{enumerate}
\begin{flushleft}
...
\item For each $\lambda_1,\lambda_2,...,\lambda_k$
  ...
  \item Repeat until convergence:
    \begin{enumerate}
    \item Get $Z^{new}$ by solving $\|P_\Omega( (Y - WA_0B') - ZB')\|_F^2 + \lambda_i\|Z\|_*$ (Algorithm \ref{alg:soft-impute})
    \item Find $A^{new}$ by solving $\|P_\Omega( (Y - Z^{new}B') - WAB')\|_F^2$ (Algorithm \ref{alg:regression})
...
\begin{figure}[h!]
  \includegraphics[width=0.85\linewidth]{images/components}
...
\begin{figure}[h!]
  \includegraphics[width=0.85\linewidth]{images/predictions}
...
\begin{figure}[h!]
  \includegraphics[width=0.85\linewidth]{images/cross-val}
