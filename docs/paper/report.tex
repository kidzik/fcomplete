...
Suppose, as previously, that the true processes are in a low-dimensional space of splines and that we observe them with noise. More precisely, let
\begin{align}\label{eq:definitions-xy}
  \bx_i' = \bw_i'B' + \be_{x,i}' \text{\ \ and\ \ } \by_i' = \bu_i'B' + \be_{y,i}',
\end{align}
...
\begin{align}\label{eq:errors-in-variables}
  \tilde{X} = XB^{-1} = W + \tilde{E}_X \text{ and } \tilde{Y} = YB^{-1} = WA + \tilde{E}_Y,
\end{align}  
...
\begin{align}\label{eq:lag:bivariate}
  \minimize_{A,W} & \frac{1}{\sigma_Y^2} \| Y - WAB' \|_F^2 + \frac{1}{\sigma_X^2}\|X - WB' \|_F^2.%,\nonumber\\
%  \subjectto & \rank(W) = k.
\end{align}
...
%  \subjectto & \rank(W) = k.
\end{align}
Without loss of generality we assume that $B$ is orthonormal. We have freedom to choose the basis $B$ and any basis can be orthogonalized using, for example, singular value decomposition. Then \eqref{eq:lag:bivariate2} is equivalent to 
\begin{align}\label{eq:lag:bivariate3}
  \minimize_{A,W} & \| (\gamma YB:XB) - W(\gamma A:I) \|_F^2.%,\nonumber\\
%  \subjectto & \rank(W) = k.
\end{align}
To solve \eqref{eq:lag:bivariate3}, we factorize $(\gamma YB: XB)$ to $\tilde{W}SV'$ using singular value decomposition. \tr{By the model assumption \eqref{eq:linear-xy}, matrix $(\gamma YB: XB)$ is rank $K$ (TODO: This is only ``almost'' true)}. Therefore, we can transform the decomposition such that $(\gamma A:I) = RV'$ and $W = \tilde{W}SR^{-1}$
...
\begin{align*}%\label{eq:lag:bivariate2-partial}
  \minimize_{A,U} & \| P_{\Omega:\Omega}((Y:\gamma X) - W(AB':\gamma B')) \|_F^2,\nonumber\\
  \subjectto & \rank(W) = k,
\end{align*}
where $k$ is the desired rank of the solution, $P_{\Omega:\Omega}$ is a projection on $(\Omega:\Omega) = \{(q,r): (q,r) \in \Omega \text{ or } (q,r-T) \in \Omega\}$. As previously for $k$ unknown we can introduce a rank penalty and we relax it to the nuclear norm
\begin{align}\label{eq:lag:bivariate2-partial}
  \minimize_{A,U} & \| P_{\Omega:\Omega}((Y:\gamma X) - W(AB':\gamma B')) \|_F^2 + \lambda\|W\|_*.
\end{align}
The truncated SVD solves the optimization problem. The algorithm in a general case of multiple processes is given in Section \ref{ss:dim-red}.

Although we motivate the problem as a regression of $Y$ on $X$, note that $X$ and $Y$ are symmetric in \eqref{eq:lag:bivariate2}. The low-rank matrix $W$ is a joint low-rank representation of matrices $X$ and $Y$ and thus our method can be seen as a dimensionality reduction technique or as a latent space model. In Section \ref{ss:dim-red} we extended this idea to a larger number of variables. In Section \ref{ss:regression} we discuss how this approach can be used for regression.

The linear structure of \eqref{eq:almost-errors-in-variables} allows us to draw analogy not only to the errors-in-variables models but also to vast literature on {\em canonical corroletaion analysis (CCA)}, {\em partial least squares (PLS)}, {\em factor analysis (FA)}, and {\em linear functional equation (LFE) models}. One can show that solving the problem \eqref{eq:lag:bivariate} is equivalent to finding the first $k$ principal directions in the canonical correlation analysis (CCA) \citep{hotelling1936relations}. Moreover, if $\gamma=1$ the problem is equivalent to PLS \citep{wold1975soft}. Solutions of CCA and PLS can be derived from the SVD of $(YB:\gamma XB)$ \citep{borga1997unified}. For discussion on relation between errors-in-variables, FA and CCA please refer to \cite{gleser1981estimation}.
...
We extend the equation \eqref{eq:lag:bivariate2} to account for a larger number of covariates
...
Finally, if measurements are on different scales, we may normalize them using scaling $\gamma_1, \gamma_2,..., \gamma_p > 0$, and taking
