...
%Note that by allowing a different basis for each process, we also allow for covariates constant in time if $X_i$ is a $N\times 1$ matrix and $B_i = \begin{bmatrix} 1 \end{bmatrix}$ for some $1 \leq i \leq p$. If measurements are on different scales, we may normalize them using scaling $\gamma_1, \gamma_2,..., \gamma_p > 0$, and taking
