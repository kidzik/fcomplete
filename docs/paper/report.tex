...
We test four methods for prediction: functional principal components (fPCA), \textsc{Soft-Longitudinal-Impute} (SLI), \textsc{Sparse-Longitudinal-Regression} (SLR), and the \emph{null model} -- the LOWESS estimation of the population mean (\verb|mean|). In SLR, we approximate GDI by using latent variables of sparsely observed covariates BMI and O2 expenditure.

We randomly select $10\%$ of observations from patients who visited the clinic at least $4$ times as our test set. Then, we split the remaining $90\%$ of observations into a training and validation sets. We train the algorithms with all combinations of parameters: the regularization parameter for SLI and SLR procedures $\lambda \in \{0, 0.1, 0.2, ..., 2.0\}$ and the rank for fPCA procedure $d \in \{2,3,4,...,K\}$. We define the grid of $T = 31$ points.

Let us denote the test set as $\Omega \subset \{1,2,...,N\} \times \{1,2,...,T\}$. We validate each model $M$, by computing the mean squared error on observed but held-out indices using the formula \eqref{eq:err}. We select the parameters using cross-validation. We estimate errors by cross-validation, sampling $95\%$ of points to train the models and leaving $5\%$ for estimating the \verb|MSE|. 
...
\begin{figure}[h!]
  \includegraphics[width=0.7\linewidth]{../plots/data-boxplot}
  \caption{Distribution of cross-validated MSE of the four procedures: functional principal components (fPCA), Soft-Longitudinal-Impute (SLI), Soft-Longitudinal-Regression (SLR).}
  \label{fig:Boxplots}
\end{figure}

Compared to the null model all three methods reduce the \verb|MSE| by $\sim 10\%$. We present detailed results in Table \ref{tbl:data-res} and in Figure \ref{fig:Boxplots}. 
