...
=======
  ...
  %\subjectto & \rank(U) = k,
>>>>>>> e80be8cf4467162a3b48a2d54aa0c7fe89f699e1
\end{align}
...
<<<<<<< HEAD
Suppose that for every individual we observe multiple variables varying in time, e.g., results of several medical tests at different times in a clinic, and we want to find a projection on $\R^d$ maximizing the variance explained for some $d \in \N$. %For fully observed $X_1,...,X_p$ this would correspond to the reduced-rank SVD of concatenated matrices $X_1~:~X_2~:~...~:~X_p$. %In this section, we introduce a method for finding such projection when the data is sparsely observed. We illustrate the method on bivariate processes, however it can be easily extended to any $p$-dimensional processes. 
=======
