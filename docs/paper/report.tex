...
For the regression problem, we suggest a two-step procedure motivated by principal component regression. We first find a joint low-rank decomposition $W$ of covariates $X_1,...,X_p$ and then regress $Y$ on $W$. Note however that this way we are not levaraging the intrinsic structure of $Y$. To incorporate the trends of variability in $Y$ independent of $X_i, 1 \leq i \leq p$ we introduce an additional latent space $U$.
...
Let $U$ be an unknown $N \times d_1$ matrix, $W$ be a $N \times d_2$ orthogonal matrix, where $d_1, d_2$ are the numbers of latent components. We are searching for $U$ and a $(d_1 + d_2) \times K$ matrix $A$
\begin{align}\label{eq:pcr}
\minimize_{U,A} \|P_\Omega(Y - (U:W)AB')\|_F^2,
\end{align}
where $P_\Omega$ is a projection on a set of observed coefficients.

We propose an iterative algorithm based on alternating least squares method (ALS). We assign random values to U and regress Y on $(U:W)$, obtaining $A$. Next, we regress $Y$ on $A$ but only allowing $U$ to change. We refer to Algorithm \ref{alg:sparse-regression} for the full description of the procedure.
