...
data = sample.long(all.data.filtered, "Patient_ID", "age", "O2cost", ratio = 0.05)
...
model.impute.fpcs = fregression(O2cost:age ~ 1 | Patient_ID, data$train, lambda= c(7.5), thresh = 1e-4, method = "fpcs", K=2, d=4)
model.mean = fregression(O2cost:age ~ 1 | Patient_ID, data$train, lambda= c(100), thresh = 1e-6, method = "mean", K=2)

# REGRESSION
# model.regression = fregression(GDI_Pre:age ~ speed + mass + height + cadence| Patient_ID, all.data, method = "fpcs", K = 3,thresh=1e-4)
model.regression = fregression(O2cost:age ~ speed + cadence + bmi + height + PC1 | Patient_ID, data$train, method = "fpcs", K = 2, thresh=1e-10, K.reg = 2, lambda.reg = c(1,3,5,10,20), d=4)
...
lambdas = 100 + c(20) * 0:20
model.impute = fregression(O2cost:age ~ 1 | Patient_ID, data$train, lambda= lambdas, thresh = 1e-10, method = "fimpute", K=2, d=4)
model.impute$meta
...
matplot(t(data$test.matrix[ind,]),t='p',lty=2,lwd=2,pch="x", ylim = c(-150,150))
matplot(t(model.regression$fit[ind,]),t='l',lty=4,add=T,lwd=2)
matplot(t(model.mean$fit[ind,]),t='l',lty=3,add=T,lwd=2)
#matplot(t(model.impute$fit[ind,]),t='l',lty=1,add=T,lwd=2)
