...
for(exp.id in 1:nexp){
  ...
  # SIMULATE DATA
  set.seed(323 + exp.id)
  simulation = fsimulate(dgrid = dgrid,clear = 0.25, n = 100,noise.mag = 0.05,K = 15)
  ...
  matplot(t(ftrue[idx,,drop=F][ind,,drop=F]),t='l',lty=1,ylim=c(-5,3),lwd=1,xlab = "time",col=1)
  #matplot(t(simulation$fobs[ind,]),t='p',lty=2,add=T,lwd=2,pch="x")
  missing = simulation$ftrue
  missing[!is.na(simulation$fobs)] = NA
  matplot(t(simulation$fobs[ind,,drop=F]),t='p',lty=2,lwd=3,pch="x",xlab = "time",add=T)
  matplot(t(missing[ind,,drop=F]),t='p',lty=2,lwd=3,pch="o",add=T)
  matplot(t(model.fimp$fit[ind,,drop=F]),t='l',lty=2,add=T,lwd=3)
  title("Functional impute for missing data")
